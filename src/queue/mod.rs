...
impl JobQueue {
    ...
    pub async fn stats(&self) -> AppResult<QueueStats> {
        let mut conn = self.cache.get_connection().await?;
        let pending: i64 = redis::cmd("LLEN").arg(JOB_QUEUE_KEY).query_async::<_, i64>(&mut conn).await?;
        let processing: i64 = redis::cmd("LLEN").arg(JOB_PROCESSING_KEY).query_async::<_, i64>(&mut conn).await?;
        let dead: i64 = redis::cmd("LLEN").arg(JOB_DEAD_KEY).query_async::<_, i64>(&mut conn).await?;
