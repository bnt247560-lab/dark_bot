...
impl Settings {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let settings: Self = Config::builder()
            ...
            .set_default("dashboard_enabled", false)?
            .set_default("dashboard_token", "")?
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        if self.dashboard_enabled {
            let token = self.dashboard_token.trim();
            if token.len() < 24 || token == "change_me_to_a_long_random_token_32_chars" {
                anyhow::bail!("DASHBOARD_TOKEN must be a non-placeholder value with at least 24 characters when DASHBOARD_ENABLED=true");
            }
        }
