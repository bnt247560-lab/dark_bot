...
This build runs SQLx migrations automatically during application startup.
On a fresh Render PostgreSQL database, the app will create the required `job_status`, `users`, and `jobs` schema before starting the bot workers.

Required files:

- `migrations/202607030001_initial_schema.sql`
- `sqlx::migrate!("./migrations")` in `src/app.rs`

You no longer need to manually execute the initial SQL in Render PostgreSQL for a fresh database.
